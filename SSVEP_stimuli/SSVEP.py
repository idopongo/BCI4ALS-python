...
ACTUAL_SCREEN_RESOLUTION = [1920,
                            1080]  # Change this to reflect the actual screen resolution, or else your stimuli will be the wrong sizes
...
class SSVEP:
    ...
    def __init__(self, mywin=visual.Window(size=ACTUAL_SCREEN_RESOLUTION, color=[138, 138, 138], colorSpace='rgb255',
                                           fullscr=False, monitor='testMonitor', units='deg'),
                 frame_off=1, target_freq=6, blockdur=5.0, port='/dev/ttyACM0',
                 fname='SSVEP', numblocks=1, waitdur=2, randomlyVarySize=False, isSinusoidalStim=True,
                 doFixationTask=True, numFixColorChanges=8, fixChangeDurSecs=0.2, minSecsBtwFixChgs=1.2,
        ...
        self.fixation = visual.TextStim(win=self.mywin, text='+', name='fixation', units='deg', height=0.5, pos=[0, 0],
        ...
        self.diodeStimulator = visual.GratingStim(win=self.mywin, name='diodeStim', units='norm',
                                                  tex=None, pos=[-1, -1], size=[0.1, 0.1], color=[0, 0, 0],
                                                  colorSpace='rgb255',
                                                  opacity=1, interpolate=True)
        ...
        self.StimulationFreq = self.frameRate / (round(
            self.frameRate / self.targFreq))  # calculate actual stimulation frequency rounded to the screen refresh rate
        ...
        if self.doFixationTask:
            ...
            for fixChangeInd in range(self.numFixColorChanges):
                thisChgFrame = random.randint(self.numFramesFixChange,
                                              totFrames - self.numFramesFixChange - self.minFramesBtwFixChgs)
                if fixChangeInd > 0:
                    moreThanOneSecDiffsFromThis = [abs(x - thisChgFrame) > self.minFramesBtwFixChgs for x in
                                                   self.fixChgFrames]
                    howManyTries = 0
                    while not all(moreThanOneSecDiffsFromThis):
                        ...
                        if howManyTries >= tooManyTries:
                            print(
                                "ERROR: You put too many fixation color change events and/or set too wide a minimum spacing between the events to actually fit in the experiment.")
        ...
        if self.randomlyVarySize:
            ...
            possiblePercentVals = range(self.sizePercentRange[0], self.sizePercentRange[1] + self.sizePercentSteps,
                                        self.sizePercentSteps)
    ...
    def Generate_stimList(self):
        self.tryNum = 1
        ListSaveName = "Stimuli_list_" + self.fname + "_" + str(
            self.tryNum) + ".txt"  # saves a list of your stimuli to be used during presentation
        ...
        NbPatterns = self.numblocks * int(math.ceil(
            self.blockdur * self.StimulationFreq / sum(self.StimPattern)))  # (NbStims / sum(self.StimPattern))

        # Get names of all stimuli in directories and shuffle the order within each stimulus type
        AllStims = []
        for e in range(len(self.StimDir)):
            AllStims.append([])
            files = [f for f in os.listdir(os.path.join(self.baseStimDir, self.StimDir[e])) if (
                    os.path.isfile(os.path.join(self.baseStimDir, self.StimDir[e], f)) and (imghdr.what(
                os.path.join(self.baseStimDir, self.StimDir[e],
                             f)) is not None))]  # get names of all stimuli in directories
            ...
            for stimType in range(len(self.StimPattern)):
                for stimRep in range(self.StimPattern[stimType]):
                    ...
                    if stimInds[stimType] >= len(AllStims[stimType]):
                        ...
                        while StimList[-1] is AllStims[stimType][
                            0]:  # while the last stimulus is the same as the next random one for this stimulus type, re-randomize so as to avoid repeats of identical stimuli
                            random.shuffle(AllStims[stimType])
    ...
    def Generate_stimListOddball(self):
        self.tryNum = 1
        ListSaveName = "OddballStimuli_list_" + self.fname + "_" + str(
            self.tryNum) + ".txt"  # saves a list of your stimuli to be used during presentation
        while os.path.isfile(ListSaveName):
            self.tryNum = self.tryNum + 1
            ListSaveName = "OddballStimuli_list_" + self.fname + "_" + str(
        ...
        AllStims.append([])
        AllStims.append([])
        files = [f for f in os.listdir(self.baseStimDir) if (os.path.isfile(os.path.join(self.baseStimDir, f)) and (
                imghdr.what(
                    os.path.join(self.baseStimDir, f)) is not None))]  # get names of all stimuli in directories
        random.shuffle(files)
        for i in range(len(files)):
    ...
    def start(self):
        ...
        if self.isSinusoidalStim:
            ...
            for frameOnNum in range(self.frame_on):
                self.OpacityScaleVals.append((math.cos(
                    self.sinPhaseStep + math.pi) + 1.0) / 2.0)  # the function is adjusted to start at a min of 0 and have a max of 1 mid-way
                self.sinPhaseStep = self.sinPhaseStep + self.sinPhaseStepSize
            self.OpacityScaleVals[self.OpacityScaleVals.index(
                max(self.OpacityScaleVals))] = 1.0  # Enforce that the max opacity scale val is always 1.0, even when the phase step size would not otherwise allow for it.  If there are two max values, it changes just the first max value to 1.  In practice though there is usually just one max value, even though with an odd number of frame_on frames that max may differ only by a practically infinitesmal amount from another number in the array.  That max is not always the first occuring between those two near identical values.
        ...
        while self.count < self.numblocks:
            ...
            while self.waitClock.getTime() < self.waitdur:
                ...
                if self.response:
                    if self.response[0] == 'escape':
                        sys.exit()
                    elif self.doFixationTask and self.response[
                        0] == self.respondChar and self.thisFixChangeInd > 0 and self.fixHasChangedThisBlock and not \
                            self.fixChgsDetected[self.thisFixChangeInd - 1]:
            ...
            while self.Trialclock.getTime() < self.blockdur:
                ...
                if self.stimNum >= totStimuliPrepped:
                    ...
                    print(
                        "This error is likely due to the actual screen refresh rate not being stable across the run.  To salvage any task data and prevent the code from breaking, no more images will be displayed for the rest of the run!")
                    self.stimulusNumberError = True
                    self.stimulusNumberErrorKind = "Too few prepped"
                    break
                self.pattern2.image = self.stimMat[self.stimNum]
                if self.randomlyVarySize:
                    self.pattern2.size = [self.stimSize[0] * self.randScalingVals[self.stimNum],
                                          self.stimSize[1] * self.randScalingVals[self.stimNum]]
        ...
        self.runInfo = ["Task data file:   " + dataSaveFile,
                        ...
                        "Stimulus size (the central size, if varying):   " + str(self.stimSize[0]) + " x " + str(
                            self.stimSize[1]),
                        "Stimulus size units:   " + self.pattern2.units,
                        "Did stimulus size vary?:   " + str(self.randomlyVarySize),
                        "Stimulus size range (percent of central size):   " + str(
                            self.sizePercentRange[0]) + " - " + str(self.sizePercentRange[1]),
...
class InputBox(object):
    ...
    def __init__(self):
        ...
        self.options = {'participant': 0, 'session': 1, 'port': '/dev/tty/ACM0',
                        'Frequency': 15, 'Duration': 15,
                        'InterBlockTime': 2, 'NumberBlocks': 1}
    ...
    def parameters_menu(self):
        ...
        if self.myDlg.OK:  # then the user pressed OK
            ...
            self.options = {'participant': self.thisInfo[0], 'session': self.thisInfo[1], 'port': self.thisInfo[2],
                            'Frequency': self.thisInfo[3], 'Duration': self.thisInfo[4],
                            'InterBlockTime': self.thisInfo[5], 'NumberBlocks': self.thisInfo[6]}
        else:
...
stimuli = SSVEP(frame_off=1, target_freq=freq, fname=filename, port=port_addr,
                blockdur=flash_dur, numblocks=blocknums, waitdur=waitduration, randomlyVarySize=True,
                isSinusoidalStim=True, doFixationTask=True, showDiodeStimulator=True)
